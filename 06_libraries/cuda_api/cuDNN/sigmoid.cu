...
// command to run: nvcc -o sigmoid sigmoid.cu -lcudnn -lcublas && ./sigmoid
...
__global__ void naive_sigmoid_kernel(float* input, float* output, int size){
...
int main(){
  ...
  for(int i=0;i<num_warmup;i++){
    naive_sigmoid_kernel<<<grid, block>>>(d_input, d_output_naive, size);
  }
  ...
  for(int i=0;i<size;i++){
    h_cpu_output[i] = cpu_sigmoid(h_input[i]);
  }
  ...
  printf("Average Naive sigmoid Time: %f ms\n", avg_naive_time);
  printf("Average cuDNN sigmoid Time: %f ms\n", avg_cudnn_time);
  printf("Max Error Naive sigmoid: %f\n", max_error_naive);
  printf("Max Error cuDNN sigmoid: %f\n", max_error_cudnn);
