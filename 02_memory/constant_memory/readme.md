...
```mermaid
flowchart LR
  A[Host coefficients] --> B[cudaMemcpyToSymbol]
  B --> C[Constant memory]
  C --> D[Constant cache per SM]
  D --> E[Warp broadcast when same address]
```
...
```mermaid
flowchart TD
  A[32 threads in warp] --> B[All read coeffs[3]]
  B --> C[Broadcast from constant cache]
  C --> D[1 memory transaction]
```
